...
func (s *portTestSuite) Test_PPUCTRL() {
	...
	s.ppu.WriteRegister(addr, 0b01010000)
	s.True(s.ppu.ctrl.slave)
	s.True(s.ppu.ctrl.bgTable)

	s.ppu.WriteRegister(addr, 0b00000111)
...
func (s *portTestSuite) Test_PPUSTATUS() {
	...
	s.EqualValues(0b11000000, s.ppu.ReadRegister(addr))
	s.False(s.ppu.w)
	s.EqualValues(0b01000000, s.ppu.ReadRegister(addr))
}

func (s *portTestSuite) Test_OAMADDR() {
	var addr uint16 = 0x2003

	s.ppu.WriteRegister(addr, 255)
...
func (s *portTestSuite) Test_OAMDATA_read() {
	...
	s.ppu.WriteRegister(0x2003, 0x09)

	s.EqualValues(0xA3, s.ppu.ReadRegister(addr))
}

func (s *portTestSuite) Test_OAMDATA_write() {
	var addr uint16 = 0x2004

	s.ppu.WriteRegister(0x2003, 0xAB)
	s.ppu.WriteRegister(addr, 0x32)
...
func (s *portTestSuite) Test_PPUSCROLL() {
	...
	s.ppu.WriteRegister(addr, 0x1F)

	s.True(s.ppu.w)
	s.EqualValues(3, coarseX(s.ppu.t))
	s.EqualValues(0b111, s.ppu.x)

	s.ppu.WriteRegister(addr, 0x0E)
	s.False(s.ppu.w)
...
func (s *portTestSuite) Test_PPUADDR() {
	...
	s.ppu.WriteRegister(addr, 0x3F)
	s.True(s.ppu.w)

	s.ppu.WriteRegister(addr, 0x91)
	s.False(s.ppu.w)
...
func (s *portTestSuite) Test_PPUDATA() {
	...
	s.ppu.WriteRegister(0x2006, 0x2F)
	s.ppu.WriteRegister(0x2006, 0x11)

	s.ppu.WriteRegister(addr, 0x83)
