...
func Test_nestest(t *testing.T) {
	...
	for sc.Scan() {
		...
		require.EqualValues(t, *li, tr, "lineno:%d %s", i, line)

		i++
	}
	require.NoError(t, sc.Err())

	assert.EqualValues(t, 26560, nes.Trace().Cycles)
...
func parseCPUTrace(line string) (*Trace, error) {
	re := nestestLogLineRe
